	...
	return (
		...
		>
			<Container fluid className="circles-section-content-container px-md-0">
					...
					<>
						...
						<div className="d-block d-md-none">
							{(() => {
								...
								const iconHoverSrc =
									item.iconHoverState?.localFile?.publicURL || item.iconHoverState?.sourceUrl;
								const iconDefaultImage = getImage(item.icon?.localFile);
								const iconDefaultSrc =
									item.icon?.localFile?.publicURL || item.icon?.sourceUrl;
							...
							<div className="d-flex justify-content-between align-items-center mt-3 px-2">
								...
								<div className="d-flex gap-2">
									<button
										className="circles-carousel-arrow"
										onClick={prev}
										aria-label="Previous"
									>
								...
								<div className="circles-peek-track">
									{[-1, 0, 1].map(offset => {
										...
										const displayImage = isActive
											? (iconHoverImage || iconDefaultImage)
											: iconDefaultImage;
										const displaySrc = !displayImage
											? (isActive ? (iconHoverSrc || iconDefaultSrc) : iconDefaultSrc)
											: null;
										const displayAlt =
											(isActive ? item.iconHoverState?.altText : null) ||
											item.icon?.altText ||
											"Service icon";
						...
						<div className="d-none d-xl-block circles-orbit">
							<div className="circles-orbit-inner">
								{sectionImage && (
									<div className="circles-orbit-center">
										<GatsbyImage
											image={sectionImage}
								...
								{service.map((item, index) => {
									...
									const isSixItemLayout = service.length === 6;
									const fallbackAngle = (360 / service.length) * index;
									const customPosition = isSixItemLayout
										? orbitPositionsForSix[index]
										: null;

									return (
										<div
											key={index}
											className={`circles-orbit-item${hasHoverIcon ? " has-hover-icon" : ""}${!isSixItemLayout ? " is-radial" : ""}`}
											style={{
												"--hover-colour": item.hoverColour,
												"--angle": `${fallbackAngle}deg`,
												...(customPosition
													? {
				...
				{(primaryCtaButton?.title || secondaryCtaButton?.title) && (
					<Row className="mt-5">
						<Col
							xs={12}
							className="text-center d-flex align-items-center justify-content-center flex-wrap gap-3"
						>
							...
							{secondaryCtaButton && secondaryCtaButton.url && (
								...
								>
									...
									<svg
										...
										height="16"
										viewBox="0 0 24 24"
										fill={secondaryCtaChevronColour || secondaryCtaTextColour}
									>
			...
			<style>{`
				...
				.circles-orbit-center {
					position: absolute;
					top: 50%;
					left: 50%;
					width: min(300px, 21vw);
					height: min(300px, 21vw);
					border-radius: 50%;
					overflow: hidden;
				...
				/* Fallback for service counts other than 6 */
				.circles-orbit-item.is-radial {
					top: 50%;
					left: 50%;
					transform:
						translate(-50%, -50%)
						rotate(var(--angle))
						translateY(calc(-1 * min(280px, 21vw)))
						rotate(calc(-1 * var(--angle)));
				}

				.circles-orbit-item-inner {
					width: 100%;
					height: 100%;
					border-radius: 50%;
					display: flex;
				...
				.circles-orbit-item-inner:hover {
					background-color: var(--hover-colour);
					border-color: transparent;
				}

				.circles-orbit-item .circles-section-service-heading {
					color: white;
					font-size: 15px;
					line-height: 1.3;
				}

				.circles-orbit-item .circles-section-service-description,
				.circles-orbit-item .circles-section-service-description p {
					color: white;
					font-size: 12px;
					line-height: 1.45;
					margin-bottom: 0;
				...
				.circles-orbit-item.has-hover-icon .circles-orbit-item-inner:hover .icon-default {
					display: none;
				}

				.circles-orbit-item.has-hover-icon .circles-orbit-item-inner:hover .icon-hover {
					display: flex;
				}
			`}</style>
